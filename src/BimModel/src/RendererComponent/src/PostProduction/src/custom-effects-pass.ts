...
export class CustomEffectsPass extends Pass {
	...
	constructor( resolution: THREE.Vector2, renderScene: THREE.Scene, renderCamera: THREE.Camera ) {
		super();
	...
	render( renderer, writeBuffer, readBuffer ) {
		...
		for ( const mesh of this.excludedMeshes ) {
			( mesh as THREE.Object3D ).visible = false;
		}
		...
		for ( const mesh of this.excludedMeshes ) {
			( mesh as THREE.Object3D ).visible = true;
		}
